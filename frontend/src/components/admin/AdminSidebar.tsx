...
import {
  LayoutDashboard, Ambulance, Wrench, AlertTriangle,
  Clock, LogOut, Home, Shield, ChevronLeft, ChevronRight,
} from "lucide-react";
...
const AdminSidebar = () => {
  ...
  return (
    <>
      {/* Sidebar — largeur 72 (ouvert) ou 16 (fermé, icons seulement) */}
      <aside
      ...
      >
        ...
        <div className={`relative flex items-center shrink-0 ${open ? "p-5 pb-3 justify-between" : "py-5 justify-center"}`}>
          {open ? (
            <>
              <div className="flex items-center gap-3 min-w-0">
                ...
                <div className="min-w-0">
                  <span className="font-display font-bold text-secondary-foreground text-sm block leading-tight whitespace-nowrap">
                    Ambulance Fès
                  </span>
              ...
              <button
                ...
                className="ml-2 p-1.5 rounded-lg text-secondary-foreground/40 hover:text-secondary-foreground hover:bg-secondary-foreground/10 transition shrink-0"
                title="Réduire"
              >
            ...
            <button
              ...
              className="w-9 h-9 rounded-xl bg-primary/10 flex items-center justify-center ring-2 ring-primary/20 hover:bg-primary/20 transition"
              title="Agrandir"
            >
        ...
        {/* Bouton toggle flèche quand fermé */}
        {!open && (
          <div className="flex justify-center shrink-0 pb-1">
            <button
              onClick={() => setOpen(true)}
              className="p-1 rounded-lg text-secondary-foreground/30 hover:text-secondary-foreground hover:bg-secondary-foreground/10 transition"
              title="Agrandir"
            >
        ...
        {open && (
          <>
            {user && (
              <p className="font-body text-xs text-secondary-foreground/40 px-6 pb-2 truncate shrink-0 whitespace-nowrap">
                {user.name}
              </p>
            )}
        ...
        <nav className="flex-1 overflow-y-auto overflow-x-hidden py-3 space-y-1 min-h-0 px-2">
          {open && (
            <p className="font-body text-[10px] uppercase tracking-[0.15em] text-secondary-foreground/30 font-semibold px-3 mb-3 whitespace-nowrap">
              Navigation
            </p>
          )}
        ...
        <div className="shrink-0 py-3 space-y-1 bg-secondary px-2">
          ...
          <Link
            to="/"
            title={!open ? "Retour au site" : undefined}
            className={`flex items-center rounded-xl font-body text-sm text-secondary-foreground/60 hover:bg-secondary-foreground/5 hover:text-secondary-foreground transition-all
              ${open ? "gap-3 px-4 py-3" : "justify-center p-3"}`}
          >
          ...
          <button
            ...
            className={`w-full flex items-center rounded-xl font-body text-sm text-secondary-foreground/60 hover:bg-destructive/15 hover:text-destructive transition-all
              ${open ? "gap-3 px-4 py-3" : "justify-center p-3"}`}
          >
      ...
      {/* Spacer qui pousse le contenu principal */}
      <div
        style={{ transition: "width 280ms cubic-bezier(0.4,0,0.2,1)", flexShrink: 0 }}
